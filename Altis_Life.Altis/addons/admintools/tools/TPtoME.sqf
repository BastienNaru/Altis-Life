...
smenu =
{
	...
	if (count plist > (_this select 1)) then {_pmenu set [(_this select 1) + 2, ["Suivant", [13], "", -5, [["expression", "snext = true;"]], "1", "1"]];}
	else {_pmenu set [(_this select 1) + 2, ["", [-1], "", -5, [["expression", ""]], "1", "0"]];};
	_pmenu set [(_this select 1) + 3, ["Quitter", [13], "", -5, [["expression", "pselect5 = 'exit';"]], "1", "1"]];
...
if (pselect5 != "exit") then
{
	_name = pselect5;
	{
		...
		{
			hint format ["Téléporte %1 à soi", _name];
